...
tebako press \
  ...
  -r|--project-root=<project-root-folder> \
...
`<bundle-package>`:: the bundle package file

`<project-root-folder>`:: the root folder of the project

`<ruby-version>`:: the Ruby version that will be packaged (optional, defaults to `3.3.7`)
...
`<tebako-runtime-package>`:: the Tebako runtime package file

`<entry-point>`:: the entry point of the application
...
`<tebako-application-package>`:: the Tebako application package file
