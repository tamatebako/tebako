...
[NOTE]
====
The first invocation of the `press` command sets up the packaging environment
and resolves the prebuilt runtime package (downloaded into the machine-wide
cache). Subsequent invocations reuse both and are much faster.
====
...
tebako press \
  ...
  [-D|--devmode] \
  [-m|--mode=<lean|fat|classic|runtime>] \
  [--image=<path>:<mount-point>] \
...
** If the `-o` option is not specified, the package is created at `<current-folder>/<entry-point-base-name>`.

** If the `-o` option is specified, the package is named according to the `-o` option.
...
`classic`::: Stitch the application image onto a prebuilt runtime package (a single-file package). The output file is named according to the `-o` option.
`runtime`::: Deprecated. Prebuilt runtime packages are produced by the https://github.com/tamatebako/tebako-runtime-ruby[tebako-runtime-ruby] pipeline (built from the pre-patched Ruby source published by https://github.com/tamatebako/ruby[tamatebako/ruby]) and are resolved automatically by the `lean`, `fat` and `classic` modes; `tebako press --mode=runtime` fails with an explanatory error.

`--image=<path>:<mount-point>`::
(optional, repeatable)
an additional image to stitch into the package; mount points must be distinct.
...
Setup provisions the prebuilt https://github.com/tamatebako/libtfs[libtfs]
package set (including the `mkdwarfs` image builder the press pipeline runs)
into the packaging environment. No Ruby is built locally: the prebuilt
runtime packages are produced by the
https://github.com/tamatebako/tebako-runtime-ruby[tebako-runtime-ruby]
pipeline and are resolved (downloaded, verified and cached) at press or
first run time.
...
{setup | press | clean}` commands via a Tebako configuration file
('tebafile').
