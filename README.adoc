...
so we encourage to use link:https://github.com/tamatebako/tebako-ci-containers[ Tebako CI docker containers] for Linux packaging.

=== Local installation

To use Tebako in MacOS or Windows, you need to install Tebako locally.

On Linux there may be cases where Docker may not be suitable for your needs, such as:
...
In such cases, you can choose to install Tebako locall on Linux as well.

Please refer to the separate document link:INSTALLATION.adoc[INSTALLATION.adoc] on how to install Tebako..
...
== Packaging Ruby Gems with Post-Installation Requirements in Tebako
