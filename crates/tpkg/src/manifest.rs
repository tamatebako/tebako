...
impl Constraint {
    ...
    /// True when the constraint is ONE exact-pin clause (`= x.y.z`, or
    /// the bare-version spelling, which the grammar reads as exact-match)
    /// — spec 03 §2.8's binding rule for a gem-closure slice. This is a
    /// grammar-level question; matching semantics live in `versions`.
    pub fn is_exact_pin(&self) -> bool {
        let s = self.0.trim();
        if s.contains(',') {
            return false;
        }
        s.starts_with('=') || s.bytes().next().is_some_and(|b| b.is_ascii_digit())
    }
}
...
pub struct AppProvides {
    ...
    /// WHERE extension slices may land (spec 03 §2.8, additive —
    /// schema_minor 10; old readers ignore the key): the declared points
    /// a slice's `augments[].extension_point` names. Absent = the payload
    /// accepts no extensions, and the dispatcher never scans for slices
    /// (spec 07 §2 step 3a gates on the key).
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub extension_points: Vec<ExtensionPoint>,
    /// The staged gem inventory (spec 03 §2.8, additive — schema_minor
    /// 10): GENERATED by the builder's `specifications/*.gemspec` scan
    /// before the image seals, never hand-authored (spec 00 §10: one
    /// generator, two consumers — the dispatcher's slice-overlap journal
    /// reads this key, the release exports the same inventory as
    /// `closure.json`). REQUIRED at authoring time when an extension
    /// point of layout `gem-home` is declared — the KEY's presence, not
    /// its length: an empty inventory (`gems: []`) is a valid statement
    /// (the base stages no gems of its own), and the option round-trips
    /// the presence exactly.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub gems: Option<Vec<GemVersion>>,
    pub capabilities: Capabilities,
}

/// An extension point's content layout (`layout:`, spec 03 §2.8).
/// Strict: an unknown spelling is a named structural error at parse
/// (the [`PayloadKind`] discipline), never a guess.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub enum ExtensionLayout {
    /// Each mounted child is a mini GEM_HOME (`gems/`,
    /// `specifications/`); composition is the language ecosystem's own
    /// multi-home search list (GEM_PATH), never a union mount — two
    /// slices carrying different versions of one gem stay
    /// visible-and-resolvable.
    GemHome,
    /// Opaque content the application enumerates itself
    /// (`<mount>/<slice>/…` — fonts, codelists).
    Files,
}

/// One `provides.extension_points[]` entry (spec 03 §2.8): WHERE an
/// extension slice lands on the base. The base declares the point; a
/// slice's `augments[].extension_point` names it; the slice mounts BELOW
/// the point at `<mount>/<slice.name>`.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ExtensionPoint {
    /// The symbolic name slices reference.
    pub name: String,
    /// VFS-absolute mount root; the slices mount below it.
    pub mount: String,
    pub layout: ExtensionLayout,
}

/// One `provides.gems[]` entry (spec 03 §2.8): one gem of the staged gem
/// home — `{name, version}`.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
/// The shared `provides.gems[]` entry checks (app and data both carry
/// the inventory — spec 03 §2.8).
fn check_gems(gems: &[GemVersion]) -> Result<(), ManifestError> {
    for gem in gems {
        check_non_empty(&gem.name, "provides.gems[].name must not be empty")?;
        check_non_empty(&gem.version, "provides.gems[].version must not be empty")?;
    }
    Ok(())
}
...
pub struct DataProvides {
    ...
    /// The staged gem inventory (spec 03 §2.8, additive — schema_minor
    /// 10) — declared by any data payload staging a gem home, a
    /// gem-carrying extension slice included: the dispatcher's
    /// slice-overlap journal (spec 07 §2 step 3a) matches the base's and
    /// the slice's inventories by name. GENERATED by the builder's
    /// `specifications/*.gemspec` scan, never hand-authored. The option
    /// round-trips the key's presence exactly (an empty inventory is a
    /// valid statement).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub gems: Option<Vec<GemVersion>>,
    pub capabilities: Capabilities,
...
impl AppProvides {
    fn validate(&self) -> Result<(), ManifestError> {
        ...
        // spec 03 §2.8 (schema_minor 10): the extension-point grammar —
        // a slice mounts at <mount>/<slice.name>, so the mount is
        // VFS-absolute, escape-free, and never "/" itself (the child
        // join must stay well-formed); a re-declared point name is an
        // authoring ambiguity, never a silent winner.
        for (i, point) in self.extension_points.iter().enumerate() {
            check_non_empty(
                &point.name,
                "provides.extension_points[].name must not be empty (slices reference it as augments[].extension_point)",
            )?;
            check_abs_path(
                &point.mount,
                "provides.extension_points[].mount must be absolute (slices mount below it at <mount>/<slice.name>)",
            )?;
            if point.mount == "/" {
                return Err(ManifestError::Invalid(
                    "provides.extension_points[].mount must not be \"/\" — the slice mounts at <mount>/<slice.name>, a child path",
                ));
            }
            if point.mount.split('/').any(|c| c == "..") {
                return Err(ManifestError::Invalid(
                    "provides.extension_points[].mount must not contain '..' components",
                ));
            }
            if self.extension_points[..i]
                .iter()
                .any(|prior| prior.name == point.name)
            {
                return Err(ManifestError::Invalid(
                    "provides.extension_points[] declares a duplicate name — an authoring ambiguity, never a silent winner",
                ));
            }
        }
        check_gems(self.gems.as_deref().unwrap_or(&[]))?;
...
// ---------------------------------------------------------------------
// AUGMENTS (spec 03 §2.8 — `augments:`, additive: schema_minor 10)
// ---------------------------------------------------------------------

/// Press-tooling provenance of a deduped slice (`built_against:`): the
/// exact base release whose gem-closure inventory the press subtracted.
/// Its PRESENCE is what makes the slice a gem-closure slice — the
/// binding rule (exact pin) applies.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct BuiltAgainst {
    /// The exact base version whose inventory was subtracted.
    pub version: String,
    /// The subtracted closure's digest (64 lowercase hex).
    pub closure_sha256: String,
}

/// One `augments:` entry (`{payload, constraint, extension_point,
/// built_against?}`) — the REVERSE dependency edge (spec 03 §2.8): this
/// slice extends the named base payload and attaches below the base's
/// declared extension point. The constraint binds against the base's
/// RESOLVED version; dispatch semantics live in spec 07 §2 step 3a.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct AugmentsEdge {
    /// The base payload's name.
    pub payload: String,
    /// The constraint against the base's RESOLVED version. A gem-closure
    /// slice (one carrying `built_against`) pins EXACTLY (`= x.y.z`) —
    /// the binding rule: subtraction is sound only against the exact
    /// inventory it subtracted. Range constraints remain legal for
    /// content that honestly spans (a `layout: files` pack carries no
    /// `built_against`).
    pub constraint: Constraint,
    /// The base's declared `extension_points[].name`.
    pub extension_point: String,
    /// Press-tooling provenance — the exact base release whose closure
    /// inventory was subtracted.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub built_against: Option<BuiltAgainst>,
}

impl AugmentsEdge {
    fn validate(&self) -> Result<(), ManifestError> {
        check_non_empty(&self.payload, "augments[].payload must not be empty")?;
        check_non_empty(
            &self.extension_point,
            "augments[].extension_point must not be empty",
        )?;
        if let Some(built) = &self.built_against {
            check_non_empty(
                &built.version,
                "augments[].built_against.version must not be empty",
            )?;
            check_sha256(
                &built.closure_sha256,
                "augments[].built_against.closure_sha256 must be 64 lowercase hex",
            )?;
            // The binding rule (spec 03 §2.8): a slice deduped against
            // the base's closure pins EXACTLY — one slice release per
...
pub struct PayloadManifest {
    ...
    /// The REVERSE dependency edge (spec 03 §2.8, additive —
    /// schema_minor 10; old readers ignore the key, the slice then
    /// simply attaches nothing): non-empty marks this payload an
    /// EXTENSION SLICE of the named base — in-process content only
    /// (`capabilities.exec: false`, enforced at validate), attaching
    /// below the base's declared extension point at dispatch (spec 07
    /// §2 step 3a). Absent = the payload augments nothing.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub augments: Vec<AugmentsEdge>,
...
impl PayloadManifest {
    ...
    /// Second (spec 03 §2.8, schema_minor 10): a declared `gem-home`
    /// extension point REQUIRES the generated `gems` inventory —
    /// consumers only ever READ `gems` for the informational overlap
    /// journal, so dispatch never refuses over it.
    pub fn validate_authoring(&self) -> Result<(), ManifestError> {
        ...
        if let Provides::App(app) = &self.provides {
            ...
            if app
                .extension_points
                .iter()
                .any(|p| p.layout == ExtensionLayout::GemHome)
                && app.gems.is_none()
            {
                return Err(ManifestError::Invalid(
    ...
    pub fn validate(&self) -> Result<(), ManifestError> {
        ...
        // spec 03 §2.8 (schema_minor 10): the reverse edge. A slice is
        // in-process CONTENT — `capabilities.exec: false`. The per-kind
        // truth tables pin app/runtime to exec: true, so the lock keeps
        // `augments` off the executable kinds (an extension carrying
        // executables rides the forward requires/expose machinery,
        // spec 30 — MECE); Provides::Other declares no capabilities and
        // has nothing to lock.
        for edge in &self.augments {
            edge.validate()?;
        }
        if !self.augments.is_empty() {
            let exec = match &self.provides {
                Provides::App(p) => Some(p.capabilities.exec),
                Provides::Runtime(p) => Some(p.capabilities.exec),
                Provides::Data(p) => Some(p.capabilities.exec),
                Provides::Toolkit(p) => Some(p.capabilities.exec),
                Provides::Other(_) => None,
            };
            if exec == Some(true) {
                return Err(ManifestError::Invalid(
...
mod tests {
    ...
    #[test]
    fn constraint_exact_pin_detection() {
        // spec 03 §2.8's binding rule: `= x.y.z` and the bare-version
        // spelling (exact-match by grammar) are exact pins; every other
        // clause shape is not.
        for pin in ["= 1.16.2", "1.16.2", "=4.0.6"] {
            assert!(Constraint::new(pin).unwrap().is_exact_pin(), "{pin}");
        }
        for open in [
            ">= 1.16, < 2.0",
            "~> 1.16",
            ...
            assert!(!Constraint::new(open).unwrap().is_exact_pin(), "{open}");
        }
    }
