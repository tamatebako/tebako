...
pub struct AppProvides {
    ...
    /// point of layout `gem-home` is declared.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub gems: Vec<GemVersion>,
...
pub struct DataProvides {
    ...
    /// `specifications/*.gemspec` scan, never hand-authored.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub gems: Vec<GemVersion>,
    pub capabilities: Capabilities,
}
...
impl PayloadManifest {
    ...
    pub fn validate_authoring(&self) -> Result<(), ManifestError> {
        ...
        if let Provides::App(app) = &self.provides {
            ...
            {
                return Err(ManifestError::Invalid(
                    "provides.gems is REQUIRED when a gem-home extension point is declared (spec 03 §2.8 — generated by the builder's specifications/*.gemspec scan; the dispatcher's overlap journal and the release's closure.json both read it)",
                ));
            }
        }
        Ok(())
