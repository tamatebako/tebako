...
| windows live capture (win-run injection + ucrt inline hooks) | NEVER smoke-tested upstream at v2.14.0 (msys.yml exercises only the library scaffold + headers); BROKEN on windows-latest in tebako CI (run 32453723170, job 96686997021): the hook installation completes in the child (`hooked 'fopen' …` diagnostics print), then `LoadLibraryA(retrace.dll)` fails inside `retrace_core_boot()` — the loader converts a DllMain crash into `ERROR_DLL_INIT_FAILED`, the inject thread's exit code reads 0, and win-run reports "failed to launch" (`win_common/inject.c:73`) | the windows libc-layer dogfood is a prerequisite probe: a standalone control isolates our half, a `RETRACE_LOGGER_RING=0` candidate pin gets the full chain gates when it captures (the MinGW build compiles the lock-free ring the MSVC build stubs out), and otherwise the leg pins the documented broken signature — drift in either direction fails it. The procmon converter (§6.2) carries the windows chain certification meanwhile |
